    ...
    public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
        ...
        try{
            ...
            if (HTTP_START.equals(className)) {
                ...
                ct.insertBefore("com.yang.monitor.record.Collector.write();");
                ...
                ct.insertBefore("String traceId = $1.getHeader(\"traceId\");" +
                        ...
                        "com.yang.monitor.agent.Interceptor.httpBegin(traceId,span,remainderString,appId,appName,method,requestURI);");
                logger.debug("start Inserted");
                ct.insertAfter("com.yang.monitor.agent.Interceptor.httpEnd();");

            //MyBatis拦截
            }else if(MYBATIS_START.equals(className)){
                cl = pool.makeClass(new ByteArrayInputStream(classfileBuffer));
                CtMethod ct = cl.getDeclaredMethod("prepare");
                ct.insertAfter("com.yang.monitor.agent.Interceptor.addSQL(delegate.getBoundSql().getSql());");
                ...
                for(CtMethod method:methods){
                    if (method.getName().equals("doBeforeInvokeReturn")){
                        method.insertAfter("String x = ($1.getAttachment(\"remainder\")==null||" +
                                ...
                                "com.yang.monitor.agent.Interceptor.addRPCTrace(" +
            ...
            //Dubbo出口
                ...
                for(CtMethod method:methods){
                    if (method.getName().equals("doBeforeInvokeReturn")){
                        method.insertBefore("com.yang.monitor.record.Collector.write();");
                        method.insertAfter("$1.setAttachment(\"traceId\", com.yang.monitor.agent.Interceptor.getTraceId());" +
                                "$1.setAttachment(\"span\", com.yang.monitor.agent.Interceptor.getSpan());" +
                                "$1.setAttachment(\"remainder\", String.valueOf(com.yang.monitor.agent.Interceptor.getRemainder()));" +
                                "$1.setAttachment(\"appId\", com.yang.monitor.agent.Interceptor.getAppId());" +
                                "$1.setAttachment(\"appName\", com.yang.monitor.agent.Interceptor.getAppName());");
                    }else if(method.getName().equals("invoke")){
                        method.insertAfter("com.yang.monitor.agent.Interceptor.endRPC();");
                    }
                }
                ...
                String code ="String traceId = com.yang.monitor.agent.Interceptor.getTraceId();" +
                        "        String span = com.yang.monitor.agent.Interceptor.getSpan();" +
                        "        int remainder = com.yang.monitor.agent.Interceptor.getRemainder();" +
                        "        String appId = com.yang.monitor.agent.Interceptor.getAppId();" +
                        "        String appName = com.yang.monitor.agent.AppInfo.getInstance().getAppName();" +
                ...
                ct.insertBefore("com.yang.monitor.agent.Interceptor.modifyDisconf($1);");
                ...
                for(CtBehavior behavior:behaviors){
                    ...
                    behavior.insertBefore("com.yang.monitor.agent.Interceptor.codisUse(\""+ behavior.getName() +"\");");
                }
